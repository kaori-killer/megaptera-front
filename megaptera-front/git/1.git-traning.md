...
git에서는 브랜치와 원격 저장소가 서로 독립적으로 존재한다. 각 브랜치는 서로 다른 원격 저장소에 연결될 수 있다.
...
git rebase 명령으로 충돌이 발생할 수 있다. 충돌을 해결하고 난 뒤, 다시 git rebase를 계속하기 위해서는 git add 후에 이 명령어를 실행한다.
