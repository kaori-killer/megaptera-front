...
캡슐화 해야 한다.) 캡슐화는 서로 연관있는 속성과 기능들을 하나의 캡슐로 만들어 데이터를 외부로부터 보호하는 것이다.

SPR을 적용하면 비슷한 무언가를 만들 때 하나의 컴포넌트만 바꿔도 원하는 것을 만들 수 있는 장점이 생긴다. 하지만 작은 컴포넌트가 많아진다면 조합의 수가 폭발적으로 많아져서 같은 UI를 구현하더라도 같은 컴포넌트를 쓰지 않아서 통일성이 떨어진다는 단점이 생긴다.
...
> 아샬님이 컴포넌트에 html 또는 자바스크립트 코드가 많아질 수도 있는데 자바스크립트 코드가 많아진다면 SAP가 아닌 다른 방법을 해보라고 하셨다. 아직은 어떤 방법을 이야기 하시는지 모르겠다.

### 2. CSS

이미 알고 있는 기준을 재활용한다. 우리는 CSS를 작성할 때 다음과 같이 컴포넌트를 분리하고 있다.
...
API에서 받아오는 데이터(JSON Schema) 기준으로 JSON이 체계적으로 되어 있다면 컴포넌트 구조에 이 구조를 이용한다. 이것에 맞춰 컴포넌트를 나누다보면 자연스레 SRP를 달성하게 되는 흐름인된다.
...
하나의 컴포넌트에 모든 것을 작성하면, 컴포넌트가 비대해지고 다양한 UI가 하나의 컴포넌트에 존재하게 된다.
