...
`Parsing error: Cannot read file '/users/sojeongyoo/documents/메가테라/6주차/상태관리_연습/tsconfig.json'.eslint`

어떤 파일을 열어도 다음과 같은 에러가 발생한다.
에러를 보면 tsconfig.json 파일은 ``상태관리_연습/External-Store/tsconfig.json'`있는데 경로를 제대로 잡지 못하고 있다.
